...
#' @param sample_col Column name for sample IDs (default: "SampleID")
#' @param plate_col Column name for plate IDs (default: "PlateID")
#' @param assay_col Column name for assays/proteins (default: "Assay")
#' @param panel_col Column name for panels (default: "Panel")
#' @return List containing sample IDs, plates, proteins, and panels
#' @export
olink_info <- function(data, sample_col = "SampleID", plate_col = "PlateID", 
                       assay_col = "Assay", panel_col = "Panel") {
    sampleIDs <- unique(data[[sample_col]])
    plates <- unique(data[[plate_col]])
    proteins <- unique(data[[assay_col]])
    panels <- unique(data[[panel_col]])
...
#' @param outlierDefX Outlier definition for x axis (default: 2.5)
#' @param outlierDefY Outlier definition for y axis (default: 4)
#' @param byPanel Boolean to plot by panel (default: TRUE)
...
#' @export
olink_pca_outliers <- function(data, outpath, outlierDefX = 2.5, outlierDefY = 4, byPanel = TRUE, ...) {
    ...
    lapply(names(pca), function(x) {
        ggplot2::ggsave(glue::glue("{outpath}/{x}_pca.pdf"), pca[[x]])
    })
    ...
    utils::write.csv(outliers, glue::glue("{outpath}/pca_outliers.csv"), row.names = FALSE)
...
#' @importFrom dplyr bind_rows filter select
#' @export
olink_umap_outliers <- function(data, outpath, outlierDefX = 2.5, outlierDefY = 4, byPanel = TRUE, ...) {
    dir.create(outpath, showWarnings = FALSE)
    ...
    lapply(names(umap), function(x) {
        ggplot2::ggsave(glue::glue("{outpath}/{x}_umap.pdf"), umap[[x]])
    })
    ...
    outliers <- lapply(umap, function(x) {
        ...
        dplyr::filter(Outlier == 1) %>%
        dplyr::select(SampleID, Outlier, Panel)
    utils::write.csv(outliers, glue::glue("{outpath}/umap_outliers.csv"), row.names = FALSE)
...
#' @param data Data frame of Olink data
#' @param outpath Output directory
#' @param plot Boolean to indicate if plots should be generated (default: TRUE)
...
#' @export
olink_lod_qc <- function(data, outpath, plot = TRUE) {
    dir.create(outpath, showWarnings = FALSE)
    lod <- data %>% dplyr::mutate(LOD_QC = NPX > LOD)
    utils::write.csv(lod, glue::glue("{outpath}/lod_qc.csv"), row.names = FALSE)
    ...
    if (plot) {
        ...
        ggplot2::ggsave(glue::glue("{outpath}/npx_hist.pdf"), p, width = 45, height = 25)
    }
...
#' @export
olink_filtering <- function(data, outpath, pca_args = list(), umap_args = list(), lod_args = list()) {
    dir.create(outpath, showWarnings = FALSE)
    message("Getting sample info")
    sample_info <- do.call(olink_info, list(data))
    writeLines(glue::glue("{names(sample_info)}: {sample_info}\n"), file.path(outpath, "sampleIDs.txt"))

    message("Running PCA")
    pca_outliers <- do.call(olink_pca_outliers, c(list(data = data, outpath = file.path(outpath, "pca")), pca_args))
    saveRDS(pca_outliers, glue::glue("{outpath}/pca/pca_outliers.rds"))

    message("Running UMAP")
    umap_outliers <- do.call(olink_umap_outliers, c(list(data = data, outpath = file.path(outpath, "umap")), umap_args))
    saveRDS(umap_outliers, glue::glue("{outpath}/umap/umap_outliers.rds"))

    message("Running LOD")
    lod_qc <- do.call(olink_lod_qc, c(list(data = data, outpath = file.path(outpath, "lod")), lod_args))
    saveRDS(lod_qc, glue::glue("{outpath}/lod/lod_qc.rds"))
    ...
    utils::write.csv(outliers, glue::glue("{outpath}/sample_outliers.csv"), row.names = FALSE)

    message("Getting proteins below the LOD")
    lod_outliers <- lod_qc$outliers %>%
        dplyr::pull(Assay) %>%
        unique()
    utils::write.csv(lod_outliers, glue::glue("{outpath}/protein_outliers.csv"), row.names = FALSE)
    ...
    utils::write.csv(data, glue::glue("{outpath}/npx_data.csv"), row.names = FALSE)

    count_table <- olink_count_table(data)
    utils::write.csv(count_table, glue::glue("{outpath}/count_table.csv"))
...
#' @param data Data frame of Olink data
#' @param conditions Condition column(s) to analyze
#' @param outpath Output directory
#' @param de_fxn Differential expression function (default: OlinkAnalyze::olink_wilcox)
#' @param gsea_fxn Pathway analysis function (default: OlinkAnalyze::olink_pathway_enrichment)
...
olink_analysis <- function(
    ...
    for (condition in conditions) {
        dir.create(glue::glue("{outpath}/{condition}"), showWarnings = FALSE)
        ...
        ggplot2::ggsave(glue::glue("{outpath}/{condition}/volcano.pdf"), p, width = 6, height = 6)
        utils::write.csv(de, glue::glue("{outpath}/{condition}/de_results.csv"), row.names = FALSE)
        de_res[[condition]] <- de
        ...
        utils::write.csv(gsea, glue::glue("{outpath}/{condition}/gsea_results.csv"), row.names = FALSE)
        p <- do.call(OlinkAnalyze::olink_pathway_visualization, list(gsea))
        ggplot2::ggsave(glue::glue("{outpath}/{condition}/gsea.pdf"), p, width = 12, height = 8)
        p <- do.call(OlinkAnalyze::olink_pathway_heatmap, list(gsea, de))
        ggplot2::ggsave(glue::glue("{outpath}/{condition}/gsea_heatmap.pdf"), p, width = 12, height = 8)
        gsea_res[[condition]] <- gsea
    }

    res <- do.call(rbind, res)
    utils::write.csv(res, glue::glue("{outpath}/experiment_summary.csv"), row.names = FALSE)
